 ...
 * <p/>
 * Only {@link LocalPlayer}s are billed, other entitys are simply teleported. The teleport itself is delegated to
 * another TeleportService.
 */
public class EconomyTeleportService extends ForwardingTeleportService {

  private final TeleportService delegate;
  private final EconomyService economyService;
  private final FeeProvider.FeeType fee;

  /**
   * Creates an instance that uses the given EconomyService to withdraw users with the given {@code fee} on sucessfull
   ...
   */
  public EconomyTeleportService(TeleportService delegate, EconomyService economyService, FeeProvider.FeeType fee) {
  ...
  public TeleportStatus teleport(LocalEntity entity, Warp warp) {
    ...
    TeleportStatus status = delegate().teleport(entity, warp);
