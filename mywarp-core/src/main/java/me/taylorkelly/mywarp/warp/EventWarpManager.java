...
/**
 * Provides an event-framework for the Warps managed by it. Warps can fire
 * {@link WarpEvent}s through this manager and the manager itself will fire
 * {@link WarpAdditionEvent}s and {@link WarpRemovalEvent}s when Warps are added
 * to or removed from it. Handlers can listen to these events by registering
 * themselves at this manager and providing the appropriate handler methods
 * annotated by {@link com.google.common.eventbus.Subscribe} annotations.
 * 
 * <p>
 * This manager fires all events in the thread it runs. Events cannot be
 * expected to be threadsafe and it lies in the responsibility of the handler to
 * manage further thread-safety if required.
 * </p>
 * 
...
public class EventWarpManager extends AbstractWarpManager {
    ...
    /**
     * Registers the given Object as a handler for events fired by this
     * WarpManager and the Warps managed by it.
     * 
     * @param handler
     *            the handler
     */
    ...
    /**
     * Unregisters the given Object as a handler for events fired by this
     * WarpManager and the Warps managed by it.
     * 
     * @param handler
     *            the handler
    ...
    /**
     * Populates this manager with the given Warps. Unlike {@link #add(Warp)}
     * this method will not fire {@link WarpAdditionEvent}s for addition of the
     * given Warps.
     * 
     * @param warps
     *            the Warps
    ...
    /**
     * Clears this manager, removing all Warps previously managed by it. Unlike
     * {@link #remove(Warp)} this method will not fire {@link WarpRemovalEvent}s
     * for the removal of the Warps.
     */
    public void clear() {
        warpMap.clear();
    }
