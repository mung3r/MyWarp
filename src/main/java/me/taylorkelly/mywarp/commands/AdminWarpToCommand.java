...
public class AdminWarpToCommand extends BasicCommand implements Command {
    ...
    public AdminWarpToCommand(MyWarp plugin) {
        ...
        setUsage("/warp player §8<" + LanguageManager.getColorlessString("help.usage.player")
                + "> §9<"
    ...
    public void execute(CommandSender sender, String identifier,
        ...
        Warp warp = CommandUtils.getWarpForUsage(sender, CommandUtils.toWarpName(args, 1));

        plugin.getWarpList().warpTo(warp, invitee);
        sender.sendMessage(LanguageManager.getString("warp.warpto.player")
                .replaceAll("%player%", invitee.getName()));
    }
}
