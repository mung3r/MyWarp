...
public class SocialCommands {
     ...
     * Creates an instance.
     * 
     * @param warpAcceptancePromptFactory
     *            the WarpAcceptancePromptFactory
     */
    public SocialCommands(WarpAcceptancePromptFactory warpAcceptancePromptFactory) {
     ...
     *            the Actor
     * @param receiver
     *            the Profile of the player who should receive the Warp
     * @param warp
     *            the Warp
     * @param giveDirectly
     *            whether the Warp should be given directly, without asking the
     *            owner for acceptance
     * @param ignoreLimits
     *            whether the limits of the new owner should be ignored
     * @throws CommandException
     *             if the owner could not be changed
     * @throws AuthorizationException
     *             if the Actor does have enough permissions
     * @throws NoSuchPlayerException
     *             if the receiver is offline and the flags to bypass this check
     *             where not given
     */
    ...
    public void give(Actor actor, Profile receiver, @Condition(Type.MODIFIABLE) Warp warp,
            @Switch('d') boolean giveDirectly, @Switch('f') boolean ignoreLimits) throws CommandException,
        ...
        Optional<LocalPlayer> receiverPlayer = MyWarp.getInstance().getOnlinePlayer(receiver);
     ...
     *            the warp
     * @throws CommandException
     *             if the Warp could not be privatized
     * @throws LimitExceededException
     *             if a Limit would be exceeded by creating the warp
     * @throws NoSuchPlayerException
     *             if the warp's creator is offline
     */
    ...
    public void privatize(Actor actor, @Condition(Type.MODIFIABLE) Warp warp) throws CommandException,
            LimitExceededException, NoSuchPlayerException {
        if (warp.isType(Warp.Type.PRIVATE)) {
            throw new CommandException("private.already-private");
        }
        Profile creator = warp.getCreator();
        Optional<LocalPlayer> creatorPlayer = MyWarp.getInstance().getOnlinePlayer(creator);
     ...
     *            the warp
     * @throws CommandException
     *             if the Warp could not be publicized
     * @throws LimitExceededException
     *             if a Limit would be exceeded by creating the warp
     * @throws NoSuchPlayerException
     *             if the warp's creator is offline
     ...
     * @param actor
     *            the Actor
     * @param inviteeIdentifier
     *            the identifier of the invitee
     * @param warp
     *            the Warp
     * @param groupInvite
     *            whether groups should be invited
     * @throws CommandException
     *             if the invitation fails
     * @throws AuthorizationException
     *             if the Actor does have enough permissions
     * @throws NoSuchProfileException
     *             if a player should be invited, but no Profile could be found
     *             for the given {@code inviteeIdentifier}
     */
    ...
    public void invite(Actor actor, String inviteeIdentifier,
            @Condition(Condition.Type.MODIFIABLE) Warp warp, @Switch('g') boolean groupInvite)
        ...
        if (groupInvite) {
            ...
            actor.sendMessage(ChatColor.AQUA
                    + MESSAGES.getString("invite.group.successful", inviteeIdentifier, warp.getName()));
        ...
        actor.sendMessage(ChatColor.AQUA
                + MESSAGES.getString("invite.player.successful", displayName, warp.getName()));
        if (warp.getType() == Warp.Type.PUBLIC) {
            actor.sendMessage(ChatColor.GRAY + MESSAGES.getString("invite.public", warp.getName()));
        }
     ...
     * @param actor
     *            the Actor
     * @param uninviteeIdentifier
     *            the identifier of the uninvitee
     * @param warp
     *            the Warp
     * @param groupInvite
     *            whether groups should be uninvited
     * @throws CommandException
     *             if the uninvitation fails
     * @throws AuthorizationException
     *             if the Actor does have enough permissions
     * @throws NoSuchProfileException
     *             if a player should be uninvited, but no Profile could be
     *             found for the given {@code uninviteeIdentifier}
     */
    ...
    public void uninvite(Actor actor, String uninviteeIdentifier,
        ...
        if (groupInvite) {
            ...
            actor.sendMessage(ChatColor.AQUA
                    + MESSAGES.getString("uninvite.group.successful", uninviteeIdentifier, warp.getName()));
            if (warp.getType() == Warp.Type.PUBLIC) {
        ...
        actor.sendMessage(ChatColor.AQUA
                + MESSAGES.getString("uninvited.player.successful", displayName, warp.getName()));
        if (warp.getType() == Warp.Type.PUBLIC) {
            actor.sendMessage(ChatColor.GRAY + MESSAGES.getString("uninvite.public", warp.getName()));
        }
