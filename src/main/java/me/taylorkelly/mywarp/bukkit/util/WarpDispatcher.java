...
/**
 * A custom Dispatcher implementation that, instead of simply throwing an
 * InvalidUsageException if the given arguments do not match any of the
 * registered sub-commands, tries to send the command caller to a Warp matching
 * these arguments.
 */
public class WarpDispatcher extends SimpleDispatcher {
    ...
    /**
     * Teleports the command caller to the Warp matching the first argument, if
     * the command caller is a player, only one argument is given and that
     * argument matches an warp usable by the command caller and all
     * requirements listed for {@link UsageCommands#to(LocalPlayer, Warp)} are
     * meat.
     * 
     * @param arguments
     *            the arguments
     ...
     *             if {@code arguments} does not have exactly one entry
     * @throws NoSuchWarpException
     *             if there is no Warp matching the argument or is usable by the
     *             command caller
     * @throws ParameterException
     *             if the given CommandLocals do not contain a mapping for the
     *             {@link me.taylorkelly.mywarp.Actor} class
     * @throws IllegalCommandSenderException
     *             if the command caller is not a player
     * @throws TimerRunningException
     *             if the command caller already has some sort of timer running
     */
    private void to(String[] arguments, CommandLocals locals) throws InvalidUsageException,
            NoSuchWarpException, ParameterException, IllegalCommandSenderException, TimerRunningException {
