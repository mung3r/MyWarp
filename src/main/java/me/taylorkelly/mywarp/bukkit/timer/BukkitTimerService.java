...
public class BukkitTimerService implements TimerService {
    ...
    private final Table<Object, Class<? extends TimerAction>, SelfRunningRunnable> runningTimers = HashBasedTable
            .create();
    ...
    public <T> void start(T timedSubject, Duration duration, TimerAction<T> action) {
        Preconditions.checkArgument(!has(timedSubject, action.getClass()).isTimerRunning(),
                "A timer of the type %s is already running for %s.", timedSubject, action.getClass()); // NON-NLS
     ...
     * 
     * @param <T>
     *            the type of the instance the timer runs on
     */
    public class SelfRunningRunnable<T> extends BukkitRunnable {
        ...
        /**
         * Creates an instance. The given Runnable will be submitted for
         ...
         * <p>
         * The returned Duration might not be entirely exact, since Minecraft's
         * internal clock may run faster or slower than the real-world time.
         * </p>
         * 
