...
public class TempConcurrentHashMap<K, V> extends ConcurrentHashMap<K, V> {
     ...
     * created with the default cleanse (600) a capacity of 1.5 times the number of mappings in the given
    ...
    /**
     * Creates a new map with the same mappings as the given map. The map is
     * created with the specified cleanse, a capacity of 1.5 times the number of mappings in the given
     * map or 16 (whichever is greater), and a default load factor (0.75) and
     * concurrencyLevel (16).
     * 
     * @param cleanse after which elements are removed from the map in bukkit-ticks
     */
    ...
    public V put(final K key, V value) {
        MyWarp.server().getScheduler()
                .runTaskLaterAsynchronously(MyWarp.inst(), new Runnable() {
                    @Override
                    public void run() {
                        remove(key);
                    }
