...
public class BukkitPlatform implements Platform {
    ...
    public EconomyService getEconomyService() {
        try {
            ...
            if (economyProvider == null) {
                plugin.getLogger()
                        .severe("Failed to hook into Vault (EconomyProvider is null). EconomySupport will not be avilable."); // NON-NLS
            } else {
                return new VaultService(economyProvider, plugin.getAdapter());
            }
        } catch (NoClassDefFoundError e) {
            plugin.getLogger()
                    .severe("Failed to hook into Vault (EconomyProviderClass not available). EconomySupport will not be avilable."); // NON-NLS
        }
    ...
    public DurationProvider getDurationProvider() {
        if (durationProvider == null) {
            durationProvider = new BukkitDurationProvider(getSettings().getTimersConfiguredDurationBundles(),
